package com.backend.domain.ai.template;
...
public class ImagePromptTemplate {
	...
	public String build(final String userPrompt) {
		validatePrompt(userPrompt);
		final String fullPrompt = String.format(TEMPLATE, userPrompt);
		log.debug("[AI] 프롬프트 생성 완료 - length: {}", fullPrompt.length());
		return fullPrompt;
	}

	private void validatePrompt(final String prompt) {
